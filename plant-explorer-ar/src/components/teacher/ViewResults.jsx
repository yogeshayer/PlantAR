...
export default function ViewResults() {
  const { data: results, isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.QuizResult.list('-created_date'),
    initialData: [],
  });
  ...
  const stats = {
    ...
    averageScore: results.length > 0
      ...
      : 0,
    uniqueStudents: new Set(results.map(r => r.student_name)).size,
    perfectScores: results.filter(r => r.score === r.total_questions).length
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-white/90 backdrop-blur border-2 shadow-xl">
        <CardHeader>
          <CardTitle className="text-2xl">Recent Quiz Results</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-center text-gray-500 py-8">
              No quiz results yet. Students will see their results here after taking quizzes!
            </p>
          ) : (
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">
                            {format(new Date(result.created_date), 'MMM d, yyyy h:mm a')}
                          </p>
                        </div>
