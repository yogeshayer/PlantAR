...
struct GardenTab: View {
    ...
    @State private var userLocation = "your area"
                ...
                if !isSignedIn {
                    ...
                    if !suggestedPlants.isEmpty {
                        VStack(alignment: .leading, spacing: 16) {
                            ...
                            Text("Likely today for \(userLocation)")
                                .font(.system(size: 15))
                                .foregroundColor(Color(white: 0.45))
                                .padding(.horizontal, 20)
...
struct SettingsTab: View {
    @EnvironmentObject var auth: AuthService
    @EnvironmentObject var persistence: PersistenceService
    @State private var showScientificNames = false
...
struct ARScanView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showPlantInfo) {
            if let plant = detectedPlant {
                MerlinStylePlantInfoSheet(plant: plant)
                    .environmentObject(persistence)
                    .environmentObject(auth)
...
struct ARViewContainer: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> ARView {
        ...
        context.coordinator.modelLoadedBinding = $modelLoaded
    ...
    class Coordinator: NSObject, ARSessionDelegate {
        ...
        var modelLoadedBinding: Binding<Bool>?
        ...
        func session(_ session: ARSession, didAdd anchors: [ARAnchor]) {
            for anchor in anchors {
                ...
                if let plant = plantDatabase.first(where: { $0.arImageReferenceName == detectedImageName }) {
                    ...
                    loadModel(for: plant, at: imageAnchor, in: arView)
                } else {
        ...
        private func loadModel(for plant: Plant, at anchor: ARImageAnchor, in arView: ARView) {
            loadedAnchors.insert(anchor.identifier)

            print("[AR] Loading model: \(plant.modelName)")

            Task {
                do {
                    let modelEntity = try await ModelEntity(named: plant.modelName)

                    await MainActor.run {
                        let scaleFactor = plant.scale * 0.01
                        modelEntity.scale = [scaleFactor, scaleFactor, scaleFactor]
                        modelEntity.orientation = simd_quatf(angle: -.pi / 2, axis: [1, 0, 0])
                        modelEntity.position.y = plant.yOffset

                        self.currentModel = modelEntity

                        let anchorEntity = AnchorEntity(anchor: anchor)
                        anchorEntity.addChild(modelEntity)
                        arView.scene.addAnchor(anchorEntity)

                        self.modelLoadedBinding?.wrappedValue = true

                        UINotificationFeedbackGenerator().notificationOccurred(.success)
                        print("[AR] Model loaded: \(plant.modelName)")
                    }
                } catch {
                    print("[AR] ERROR loading \(plant.modelName): \(error)")
                }
            }
        }

        @objc func handlePinch(_ gesture: UIPinchGestureRecognizer) {
            guard let model = currentModel, gesture.state == .changed else { return }
